...
Add CLI progress rendering that uses known Source Item totals when the runtime provides them, supports lower-bound total hints, and preserves honest unknown-total progress output.

The CLI currently has progress output but no progress bar. This slice should add determinate progress bars only for known totals. Lower-bound and unknown totals should continue to show activity, processed count, outcome counters, and Source Cursor Window checkpoints without percentages, ETAs, or `x / y` completion claims. Lower-bound totals may display a `minimum+` hint.
...
- [x] Interactive CLI progress renders lower-bound totals as `minimum+` without a progress bar or percentage.
...
- [x] `--progress log` includes total information when the total is known or lower-bound.
...
- [x] `run --all` resets known, lower-bound, or unknown total display for each active Migration Definition.
- [x] No third-party progress bar dependency is added.
- [x] Tests cover known totals, lower-bound totals, unknown totals, zero totals, `--progress log`, non-TTY output, `--progress none`, success cleanup, and failure cleanup.
...
- Lower-bound totals render as `minimum+` without a progress bar or percentage.
