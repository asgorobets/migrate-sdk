...
- [x] A Document Fetcher contract exists with a cursor schema and a read
      operation that returns a resource, optional next fetcher cursor, and
      optional resource fingerprint.
- [x] A Document Fetch Result contract exists and is reusable by future fetcher
      helpers.
- [x] A file text fetcher helper reads local text resources through Effect
      platform filesystem and path services.
- [x] The file text fetcher does not validate document schema or parse JSON.
- [x] The file text fetcher returns a stable fingerprint for the resource it
      read.
- [x] File read failures are normalized into source plugin failures with useful
      file/resource context.
- [x] The helper supports dependency injection of platform services for tests.
- [x] Tests cover successful file reads, missing/unreadable file behavior,
      fingerprint stability, and injected platform services.
- [x] The fetcher API stays separate from lookup, selector, identity, and
      version behavior.
